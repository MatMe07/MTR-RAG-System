...
class QueryParser:
    ...
    def _extract_on_stock(text: str) -> Optional[bool]:
        ...
        «есть» засчитывается только в складском контексте («есть на складе»,
        «в наличии»); «в схеме есть …», «есть на участке/установлен» наличием на
        складе не являются (иначе ADD_COMPONENT/описания схем давали on_stock=True).
        """
        text_lower = text.lower()
        if re.search(r'(?:нет|нету|нет в наличии|отсутств\w+|не имеется|не хватает)[^.!?;]{0,40}(?:на складе|на склад|в наличии)', text_lower):
            return False
        if re.search(r'(?:на складе|на склад|в наличии)(?:[^.!?;]{0,40})?(?:есть|имеется)', text_lower):
            return True
        if re.search(r'(?:есть|имеется)[^.!?;]{0,40}(?:на складе|в наличии)', text_lower):
            return True
        return None
