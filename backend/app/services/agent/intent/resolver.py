...
_MAINTENANCE_STRONG = (
    ...
    # Активные действия ТОиР, а не пассивная проверка наличия на складе.
    "порядок ремонта", "порядок замен", "ремкомплект",
    "комплект для ремонт", "комплект для замен", "комплект ремонта",
    "комплект замен", "собери комплект", "собрать комплект",
    "единый комплект", "полный комплект замен",
    "что проверить", "что подготовить", "распиши что проверить",
    "подготовь список",
)
...
def resolve_top_level_intent(
    ...
    # Смена среды/влияние важнее плана: перевод участка на агрессивную среду —
    # это анализ и перепроверка, а не просто план работ.
    if "impact_analysis" in mapped and getattr(parsed, "proposed_changes", {}):
        return "impact_analysis"
    # Классический спор «план/комплект» (maintenance) против «наличие» (inventory)
    # решается лексикой: активные действия ТОиР перевешивают проверку склада.
