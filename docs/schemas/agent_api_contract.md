...
Для проверки парсера и показа пользователю backend должен в дальнейшем
добавить в оба ответа поле `parsed_query`. До его появления поле считается
необязательным и интерфейс не падает при его отсутствии.
