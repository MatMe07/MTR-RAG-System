...
| Qdrant: фактическая миграция данных норм в `norm_documents` | **Выполнено** — 25 фрагментов норм засеяны, коллекция writable; сетд выполняется из провайдера; тест `test_norms_via_qdrant` зелёный. | Закрыто (был остаток «загрузка требует проверки»). |

### Прочее

| План | Реализация | Решение |
|---|---|---|
| LLM-доступ (живой OpenRouter) | **Ограничение окружения сохраняется.** Генерация `POST /chat/completions` → **403 "Access denied by security policy"** (блок по региону/IP/key); `/api/v1/models` также может отдавать 403. Код сохраняет graceful-fallback: запуск `auto_llm` отрабатывает без LLM (refine → review-флаг `llm_refine_failed`), детерминированный контур не ломается. Кеш LLM переведён на детерминированный ключ `sha1(prompt)`. Живые прогоны — при доступе с разрешённой сети или альтернативном провайдере (это внешний блокер, не дефект кода). |
