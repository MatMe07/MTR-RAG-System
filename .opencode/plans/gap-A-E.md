...
- Этап 1 (парсинг): РЕАЛИЗОВАНО (2026-08-29). HybridParser (9 парсеров + нормализаторы), ParsedQuery; матрица 24 интентов (INTENT_REQUIREMENTS, INCOMPATIBLE_INTENTS, BLOCKER_FIELDS — в intent/matrix.py), filter_params_for_intent + determines_parsed_status (1H, detect.py), поля ParsedQuery.intents/status/missing_params/params объявлены в схеме (аддитивно, синхронизировано); диалоговое уточнение 1G — POST /search/clarify (ClarificationManager, до 3 циклов → REQUIRES_EXPERT). DictionaryManager/БД-словари (1J/1K/1L): РЕАЛИЗОВАНО — DynamicRules (keywords/synonyms/константы/правила/overrides), group_keywords в парсере, Redis-снапшот, аудит изменений (1K.6), автопредложение (1L.5), тесты test_dictionary_manager (8; полный прогон 219 passed, 12 skipped, 8 subtests).
...
- Этап 2 (DAL): фасад IRepository + провайдеры полного стека (PG catalog/stock, Neo4j graph, Qdrant norms, passport), Redis-кеш (catalog/stock/graph), data_access_logs + access_context, fallback на JSON — реализованы ранее. detail_level (2C) и импорт (2E: REST /admin/imports/catalog|stock|graph + ImportService с историей mtr_item_history, инвалидацией кеша и аудитом) РЕАЛИЗОВАНО (2026-08-29); полный прогон 219 passed. НЕ реализованы: async-DAL (не требуется — стек синхронный), миграции 2E.6.
